...
# БАЗАИ САВОЛҲО БО АНГЛИСӢ ВА ҚОИДАҲО БО ТОҶИКӢ
QUIZ_DATA = {
    "A1 (Beginner)": [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Маълумот бо англисӣ ва номи Абдураҳим Шералиев
    welcome_text = (
        "👋 Welcome to the English Quiz Bot!\n\n"
        "👤 **Developer:** Abdurahim Sheraliev\n"
        "📚 This bot will help you test your English language levels.\n"
        "📝 The quiz consists of 10 questions. At the end, your mistakes will be explained in Tajik.\n\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("set_lvl:"):
        ...
        USER_DATA[user_id]["questions"] = all_q[:10]
        USER_DATA[user_id]["current_q"] = 0
        USER_DATA[user_id]["score"] = 0
        USER_DATA[user_id]["wrong_answers"] = []
        
        await query.edit_message_text(f"🏁 You have chosen **{level}**. The quiz has started!")
...
async def send_question(query, user_id):
    ...
    if current_q >= len(q_list) or current_q >= 10:
        await show_results(query, user_id)
        return

    question = q_list[current_q]
    text = f"❓ **Question {current_q + 1}/10:**\n\n{question['q']}"
